...
MODULE_KEYS = {
    ...
    "vulnerable_assessment": [
        "vulnerabilities", "vulns", "vulnerability_assessment", "vulnerability-assessment",
        # sensitive / google dorks
        "sensitive", "google_dorks", "google-dorks",
        "password_dork", "password-dorks", "password_dorks",
        "confidential_dork", "confidential-dorks",
        "uncommon_ext_dork", "uncommon-ext-dork", "uncommon_ext_dorks",
        # wayback machine buckets (all 7)
        "wayback", "wayback_machine", "wayback-machine",
        "xls_urls", "xml_urls", "xlsx_urls",
        "json_urls", "pdf_urls", "php_urls", "war_urls",
        # broken links / risk / social links
        "risk", "broken_links", "broken-link", "broken-links",
        "social_links", "social-link", "social-links", "domain",
        # vulnerable urls / gau
        "vulnerable_urls", "vulnerable-urls", "gau_urls", "gau-urls"
    ],
    "technology-profile-mapping": [
        "bucket", "Sbucket", "whois", "ip_ranges",
        "s3_buckets", "cve_ids", "tech_scans", "shodan",
        "phishing_vectors",
        "spf", "dmarc", "dkim",
        "emailsecurity",
        "risk", "clickjacking"
...
def normalize_techprofile_data(module_data):
    ...
    ansi_escape = re.compile(r'\x1B[@-_][0-?]*[ -/]*[@-~]')  # remove ANSI codes
    tech_stack = []

    for t in tech_stack_raw:
        match = re.match(r'^(.*?)\s*(\[.*\])$', t)
        if match:
            url, stack = match.groups()
            tech_name = ansi_escape.sub('', stack)  # remove ANSI
            tech_stack.append({"subdomain": url.strip(), "stack": [tech_name.strip()[1:-1]]})
        else:
    ...
    tech_stack = remove_duplicates(tech_stack)

    # --- Prepare tech_counts for pie chart ---
    tech_list = []
    for t in tech_stack:
        for tech in t["stack"]:
            if tech:
    ...
    if isinstance(emailsec, dict):
        ...
        phishing_vectors["emailsecurity"]["spf"] = remove_duplicates(spf_list)
        ...
        phishing_vectors["emailsecurity"]["dmarc"] = remove_duplicates(dmarc_list)
        ...
        phishing_vectors["emailsecurity"]["dkim"] = remove_duplicates(dkim_list)
    ...
    return {
        ...
        "tech_counts": tech_counts,  # for pie chart
...
def normalize_vulnerable_assessment(data: dict) -> dict:
    ...
    normalized_vulnerable_urls = {
        "gau_urls": vulnerable_urls.get("gau_urls", []) or [],
    }
    ...
    return {
        ...
        "vulnerable_urls": normalized_vulnerable_urls,
    }
...
def vuln_data(section):
    ...
    if section in ["password_dork", "confidential_dork", "uncommon_ext_dork"]:
        return jsonify(results["sensitive"]["google_dorks"].get(section, []))

    # Handle Wayback
    if section in results["sensitive"]["wayback_machine"]:
        return jsonify(results["sensitive"]["wayback_machine"][section])

    # Handle Broken Links (social links only)
    if section == "social_links":
        return jsonify(results["risk"]["social_links"])

    # Handle Vulnerable URLs
    if section == "gau_urls":
        return jsonify(results["vulnerable_urls"]["gau_urls"])
